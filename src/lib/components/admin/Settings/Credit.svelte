		...
		{#if config}
			<div>
				...
				<div class="mb-3">
					...
					<div class="text-xs text-gray-400 dark:text-gray-500">
						{$i18n.t('JSON array configuration for custom function/tool billing patterns')}
					</div>
					<hr class="mt-2.5 border-gray-100 dark:border-gray-850 my-2" />
					<div class="mt-2">
						<textarea
							...
							placeholder={$i18n.t('Enter JSON array configuration, e.g. [{"path": "$.tools[*].type", "value": "web_search_preview", "exists": false, "cost": 1000000}]')}
							rows="4"
						/>
					</div>
				</div>
